...
@section('content')
                    ...
                    <tr>
                        ...
                        <td>
                            @if (auth()->user()->id == $value->user->id)
                                <button class="btn btn-primary"><a href="{{ url('/edit/' . $value->id) }}"
                                        class="text-white"><span class="fas fa-pencil ml-3"></a></button>
                                <button class="btn btn-danger" data-bs-toggle="modal" data-bs-target="#exampleModal"><span
                                        class="fas fa-trash ml-3"></a></button>
                                <!-- Modal -->
                                    ...
                                    <div class="modal-dialog">
                                        <div class="modal-content">
                                            ...
                                            <div class="modal-footer">
                                                ...
                                                <button type="submit" onclick="Delete({{$value->id}})"
                                                    class="btn btn-danger">Confirm</button>
                                                <input type="hidden" name="id" id="deteleId{{$value->id}}" value="{{ $value->id }}" />
                                            </div>
                                        </div>
                                    </div>
    ...
    <script>
        function Delete(deletedid){
            alert(deletedid);
            var element='deletedid'+deletedid;
            alert(element);
            var id=document.getElementById('deteleId'+deletedid).value;
            alert(id);
        }
    </script>
@endsection
